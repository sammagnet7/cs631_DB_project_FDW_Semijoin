...
// --------------------------------------------- Following functions were made non static and declared here---------------------------------------
RelOptInfo *create_distinct_paths(PlannerInfo *root,
...
void copy_rte(PlannerInfo *root)
{
	size_t size = root->simple_rel_array_size - 1;
	for (Index rt = 1; rt < root->simple_rel_array_size; rt++)
	{
		if (root->simple_rte_array[rt]->relkind == 'r')
		{
			root->simple_rte_array[size] = root->simple_rte_array[rt];
			return;
		}
	}
}
// convert a List of TargetEntry to List of Var
void extract_var_list(List *list_tte, List **list_var, Index varno)
{
	ListCell *lc;
	foreach (lc, list_tte)
	{
...
void set_pathtargets_for_distinct(PlannerInfo *root, RelOptInfo *rel, List *attrs_tte)
{
	PathTarget *final_target;
				 ...
				 Index rti, RangeTblEntry *rte)
{
	...
	else
	{
		...
		case RTE_RELATION:
			if (rte->relkind == RELKIND_FOREIGN_TABLE)
			{
				if (is_join_semi(root))
				{
					bool sortable = true;
					int varno = root->simple_rel_array_size - 1;
					copy_rte(root);
					RelOptInfo *local_scan_rel = build_simple_rel(root, varno, NULL);
					...
					extract_var_list(join_attrs_tte, &join_attrs_var, varno);
					add_vars_to_targetlist(root, join_attrs_var, bms_make_singleton(1));
					...
					RelOptInfo *distinct_rel = create_distinct_paths(root, local_scan_rel);
					set_cheapest(distinct_rel);
					root->simple_rel_array[varno] = distinct_rel;
				}
				/* Foreign table */
