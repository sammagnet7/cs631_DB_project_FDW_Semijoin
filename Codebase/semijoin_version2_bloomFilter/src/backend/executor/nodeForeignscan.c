...
void appendSlotValuetoString(StringInfoData *result, TupleTableSlot *slot)
{
	...
	for (int i = 0; i < natts; i++)
	{
		...
		Oid attr_oid = typeinfo->attrs[i].atttypid;
		...
		case FLOAT4OID:
		{
			float4 num = DatumGetFloat4(value);
...
NameData get_scan_attribute(ForeignScanState *node)
{
	...
	foreach (lc, foreignScan->scan.plan.targetlist)
	{
		...
		if (IsA(tle->expr, Var))
		{
			...
			Oid vartype = var->vartype; /* OID of the attribute's data type */
